    ...
    <div className="min-h-screen select-none bg-[#F3F4F6] px-4 py-6 font-sans sm:py-12">
      <div className="mx-auto max-w-xl space-y-6">
        ...
        <section className="space-y-4 rounded-3xl border border-gray-200 bg-white p-6 text-center shadow-sm sm:p-8">
          <div className="mx-auto flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-white ring-4 ring-[#ffcf4a]">
          ...
          <button type="button" onClick={openNewComposer} className="mx-auto flex cursor-pointer items-center gap-2 rounded-full border-2 border-[#171714] bg-[#171714] px-6 py-3 text-xs font-extrabold text-white shadow-[3px_3px_0_#ff5f35] transition hover:-translate-y-0.5">
        ...
        <section className="space-y-4">
          ...
          {visibleEntries.length === 0 ? (
            ...
            return (
              <article key={entry.id} className={clsx('space-y-3 rounded-3xl border bg-white p-5 shadow-sm transition hover:border-[#ff5f35]', entry.isHidden ? 'border-dashed border-amber-300 opacity-75' : 'border-gray-200')}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-2">
                    <div className={clsx('flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full text-xs font-bold', entryIsOwner ? 'bg-amber-50 ring-2 ring-amber-300' : 'bg-[#ffcf4a] text-[#171714]')}>
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    {(canAccountEdit || canPasswordEdit) && <button type="button" onClick={() => openEditComposer(entry)} title="편집" className="cursor-pointer rounded-full p-2 text-gray-400 transition hover:bg-[#f4f1e8] hover:text-[#ff5f35]"><Pencil className="h-3.5 w-3.5" /></button>}
                ...
                {entryReplies.length > 0 && <div className="ml-10 space-y-2 border-l-2 border-[#ffcf4a] pl-3">{entryReplies.map((reply) => (
                  <div key={reply.id} className="rounded-2xl bg-gray-50 px-3.5 py-3">
                    <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><div className={clsx('flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full text-[9px] font-black', reply.authorUid === targetOwnerUid ? 'bg-amber-50 ring-1 ring-amber-300' : 'bg-[#ffcf4a] text-[#171714]')}>{reply.authorPhotoUrl ? <img src={reply.authorPhotoUrl} alt={reply.authorName} className="h-full w-full object-cover" /> : reply.authorName.charAt(0)}</div><span className="truncate text-[11px] font-extrabold text-gray-800">{reply.authorName}</span>{reply.authorUid === targetOwnerUid && <Crown className="h-3.5 w-3.5 shrink-0 fill-amber-400 text-amber-600" aria-label="주인장" />}</div><div className="flex shrink-0 items-center gap-1"><span className="text-[9px] text-gray-400">{reply.createdAt?.seconds ? new Date(reply.createdAt.seconds * 1000).toLocaleDateString() : '방금 전'}</span>{((currentUser && reply.authorUid === currentUser.uid) || (!currentUser && reply.hasEditPassword)) && <button type="button" onClick={() => { setEditingReply(reply); setReplyingToId(entry.id); setReplyAuthorName(reply.authorName); setReplyContent(reply.content); setReplyEditPassword(''); }} title="답글 편집" className="cursor-pointer rounded-full p-1.5 text-gray-400 transition hover:bg-[#f4f1e8] hover:text-[#ff5f35]"><Pencil className="h-3 w-3" /></button>}{isProfileOwner && <button type="button" disabled={busyReplyId === reply.id} onClick={() => void handleDeleteReply(reply)} title="답글 삭제" className="cursor-pointer rounded-full p-1.5 text-gray-400 transition hover:bg-red-50 hover:text-red-600"><Trash2 className="h-3 w-3" /></button>}</div></div>
                    <p className="mt-1.5 whitespace-pre-wrap pl-8 text-[11px] font-medium text-gray-700">{reply.content}</p>
                  </div>
                ))}</div>}

                <div className="flex justify-end"><button type="button" onClick={() => { setReplyingToId(replyingToId === entry.id ? null : entry.id); setEditingReply(null); setReplyAuthorName(''); setReplyContent(''); setReplyEditPassword(''); }} className="flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-bold text-gray-500 transition hover:bg-[#f4f1e8] hover:text-[#ff5f35]">{replyingToId === entry.id ? <X className="h-3.5 w-3.5" /> : <Reply className="h-3.5 w-3.5" />} {replyingToId === entry.id ? '취소' : `답글${entryReplies.length ? ` ${entryReplies.length}` : ''}`}</button></div>

                {replyingToId === entry.id && <form onSubmit={(event) => handleReplySubmit(event, entry.id)} className="ml-10 space-y-2 rounded-2xl border border-[#171714]/15 bg-[#f4f1e8] p-3">
                  <p className="text-[10px] font-bold text-[#ff5f35]">{editingReply ? '답글 편집' : currentUser ? `${signedInName}(으)로 답글 작성` : '답글 작성'}</p>
                  {!currentUser && editingReply?.authorName === '익명' ? <div className="rounded-xl border border-gray-200 bg-gray-100 px-3 py-2.5 text-[11px] font-bold text-gray-500">작성자: 익명 · 이름은 변경할 수 없습니다.</div> : !currentUser && <input type="text" value={replyAuthorName} onChange={(event) => setReplyAuthorName(event.target.value)} placeholder="미입력 시 익명" maxLength={50} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-base font-semibold outline-none focus:border-[#ff5f35]" />}
                  <textarea value={replyContent} onChange={(event) => setReplyContent(event.target.value)} placeholder="답글을 입력해주세요" maxLength={1000} rows={2} className="w-full resize-none rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-base font-semibold outline-none focus:border-[#ff5f35]" />
                  {!currentUser && <div className="relative"><KeyRound className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-400" /><input type="password" value={replyEditPassword} onChange={(event) => setReplyEditPassword(event.target.value)} placeholder={editingReply ? '작성할 때 설정한 비밀번호' : '편집 비밀번호 4자 이상'} minLength={editingReply ? undefined : 4} required className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-9 pr-3 text-base font-semibold outline-none focus:border-[#ff5f35]" /></div>}
                  <div className="flex items-center justify-between">{editingReply ? <button type="button" disabled={submittingReplyId === entry.id} onClick={() => void handleDeleteOwnReply(entry.id)} className="flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-2 text-[11px] font-extrabold text-red-600 transition hover:bg-red-50"><Trash2 className="h-3 w-3" />삭제</button> : <span />}<button type="submit" disabled={!replyContent.trim() || submittingReplyId === entry.id} className={clsx('flex items-center gap-1.5 rounded-full px-4 py-2 text-[11px] font-extrabold transition', replyContent.trim() ? 'cursor-pointer bg-[#171714] text-white hover:bg-black' : 'cursor-not-allowed bg-gray-200 text-gray-400')}><Send className="h-3 w-3" />{editingReply ? '변경 저장' : '답글 등록'}</button></div>
                </form>}
              </article>
            );
          })}
