...
import {
  ...
  CreditCard, ExternalLink, FileStack, Gift, KeyRound, LayoutDashboard,
...
const planClass = (plan: string) => plan === 'premium'
  ...
  : plan === 'standard' ? 'bg-[#ffcf4a] text-[#171714]' : 'bg-slate-100 text-slate-600';
...
const MetricCard = ({ label, value, caption, icon: Icon, tone = 'slate' }: {
  label: string; value: string | number; caption?: string; icon: React.ElementType; tone?: 'slate' | 'orange' | 'emerald' | 'amber';
}) => {
  const tones = {
    slate: 'bg-slate-100 text-slate-700', orange: 'bg-[#ffcf4a] text-[#171714]',
  ...
  return <div className="min-h-screen bg-[#f5f6f8] text-slate-950">
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-18 max-w-[1440px] items-center gap-3 px-4 sm:px-7">
        ...
        <button type="button" onClick={() => void load()} disabled={loading} className="ml-auto flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black hover:bg-slate-100 disabled:opacity-50"><RefreshCw className="h-4 w-4" /><span className="hidden sm:inline">새로고침</span></button>
      </div>
    </header>
      ...
      <main className="min-w-0 space-y-6">
        ...
        {tab === 'overview' && <>
          ...
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard label="베타 참여자" value={`${betaMembers}명`} caption={`활성 초대 ${activeInvites}개`} icon={ShieldCheck} tone="orange" />
          ...
          <section className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
            ...
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-5 space-y-4">{(['basic', 'standard', 'premium'] as const).map((plan) => {
                ...
                return <div key={plan}><div className="flex items-center justify-between text-xs"><span className="font-black">{planLabel(plan)}</span><span className="font-bold text-slate-400">{count}명 · {ratio}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${plan === 'premium' ? 'bg-amber-400' : plan === 'standard' ? 'bg-[#ff5f35]' : 'bg-slate-400'}`} style={{ width: `${ratio}%` }} /></div></div>;
              })}</div>
            </article>
          </section>

          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between"><h3 className="font-black">확인할 항목</h3><button type="button" onClick={() => setTab('members')} className="cursor-pointer text-xs font-black text-[#ff5f35]">가입자 보기</button></div>
          ...
          {membershipTransfers.filter((order) => order.status === 'WAITING_DEPOSIT' || order.status === 'DEPOSIT_REPORTED').length > 0 && <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm"><div className="flex items-center gap-3"><CalendarClock className="h-5 w-5 text-amber-700" /><div><h3 className="font-black">플랜 입금 확인 대기</h3><p className="text-[11px] font-semibold text-amber-800">실제 입금 내역과 입금자명을 확인한 뒤 처리해주세요.</p></div></div><div className="mt-4 space-y-2">{membershipTransfers.filter((order) => order.status === 'WAITING_DEPOSIT' || order.status === 'DEPOSIT_REPORTED').map((order) => <div key={order.orderNumber} className={`flex flex-wrap items-center justify-between gap-3 rounded-2xl p-4 ${order.status === 'DEPOSIT_REPORTED' ? 'border border-emerald-200 bg-emerald-50' : 'bg-white'}`}><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-black">{order.planName || order.productName} · {formatMoney(order.amount)}</p>{order.status === 'DEPOSIT_REPORTED' && <span className="rounded-full bg-emerald-600 px-2 py-1 text-[9px] font-black text-white">구매자 확인 요청</span>}</div><p className="mt-1 text-[10px] font-bold text-slate-500">입금자 {order.depositorName} · {order.buyerContact} · {order.orderNumber}</p></div><div className="flex gap-2"><button type="button" disabled={workingId === order.orderNumber} onClick={() => void handleMembershipTransfer(order, 'confirm')} className="cursor-pointer rounded-full bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:opacity-50">입금 확인</button><button type="button" disabled={workingId === order.orderNumber} onClick={() => void handleMembershipTransfer(order, 'cancel')} className="cursor-pointer rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-500 disabled:opacity-50">취소</button></div></div>)}</div></section>}
        </>}
        ...
        {tab === 'beta' && <>
          ...
          <section className="grid gap-4 md:grid-cols-2">
            {invites.map((invite) => <article key={invite.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#171714] bg-[#ffcf4a] text-[#171714]"><KeyRound className="h-5 w-5" /></span><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-black">{invite.label}</h3><button type="button" onClick={() => navigator.clipboard.writeText(invite.code)} className="mt-1 flex cursor-pointer items-center gap-1 font-mono text-xs font-bold text-slate-500 hover:text-black">{invite.code}<Clipboard className="h-3.5 w-3.5" /></button></div><button type="button" onClick={() => void toggleInvite(invite)} disabled={workingId === invite.id} className={`cursor-pointer rounded-full px-3 py-1.5 text-[10px] font-black ${invite.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{invite.status === 'active' ? '사용 중' : '중지됨'}</button></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-400">사용</span><strong className="mt-1 block">{invite.useCount} / {invite.maxUses}명</strong></div><div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-400">만료</span><strong className="mt-1 block truncate">{invite.expiresAt ? formatDate(invite.expiresAt) : '제한 없음'}</strong></div></div></article>)}
