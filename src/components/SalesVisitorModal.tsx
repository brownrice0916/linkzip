          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {lookupResults?.map((order) => <div key={order.orderNumber} className="rounded-2xl border border-gray-200 p-4 text-xs">
                ...
                <div className="mt-3 flex flex-wrap gap-2"><span className="rounded-lg bg-blue-50 px-2 py-1 font-bold text-blue-700">{paymentLabel[order.status]}</span><span className="rounded-lg bg-emerald-50 px-2 py-1 font-bold text-emerald-700">{order.salesType === 'digital_file' && order.status === 'paid' ? '다운로드 가능' : fulfillmentLabel[order.fulfillmentStatus]}</span></div>
