        ...
        {MEMBERSHIP_PLANS.map((plan) => {
          ...
          return (
            <article key={plan.id} className={`relative flex min-h-[360px] flex-col rounded-[24px] border p-5 ${styles.card}`}>
              ...
              <div className="mt-5 flex items-end gap-1 text-gray-950"><strong className="text-3xl font-black tracking-tight">{plan.monthlyPrice.toLocaleString()}</strong><span className="pb-1 text-[11px] font-bold text-gray-500">{isKo ? '원 / 월' : 'KRW / mo'}</span></div>
              <ul className="mt-5 flex-1 space-y-3">
                {(isKo ? plan.featuresKo : plan.features).map((feature) => (
                  <li key={feature} className="flex items-center gap-2 text-xs font-bold text-gray-700"><Check className={`h-4 w-4 shrink-0 ${styles.check}`} />{feature}</li>
                ))}
              </ul>
