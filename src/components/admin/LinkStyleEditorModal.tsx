...
import { ArrowLeft, ChevronDown, Link2, RotateCcw, Type } from 'lucide-react';
...
const getDefaults = (variant: EditorVariant, theme: ThemeDefaults): EffectiveDefaults => {
  ...
  return {
    ...
    // Every block surface follows the page-level button roundness by default.
    // Calendar and map used to be fixed at 16px, which made the design control
    // appear broken for those block types.
  ...
  return (
    <div className="space-y-7 animate-fade-in pb-20 font-sans">
      <header className="flex items-center gap-4">
        ...
        <div className="min-w-0"><h2 className="text-2xl font-black text-gray-950">{designOnly && isCollection ? '컬렉션 전체 디자인' : designOnly ? '개별 디자인' : isCollection ? '컬렉션 상세' : '링크 상세'}</h2><p className="text-xs font-bold text-[#ff5f35] mt-0.5">{variantLabel[variant]}</p></div>
      </header>
      ...
      {!designOnly && <section className="bg-white rounded-3xl border border-gray-100 shadow-xs p-6 space-y-4">
        ...
        {!isCollection && <label className="block space-y-2">
          ...
          <input type="text" value={link.title || ''} onChange={(event) => onUpdate({ title: event.target.value })} placeholder="링크 제목을 입력하세요" className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-3.5 text-sm font-bold text-gray-900 focus:border-[#ff5f35] focus:outline-none focus:ring-2 focus:ring-[#ff5f35]/20" />
        </label>}
        {variant !== 'map' && <label className="block space-y-2">
          ...
          <input type="url" value={link.url || ''} onChange={(event) => onUpdate({ url: event.target.value })} placeholder="https://example.com" className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-3.5 text-sm font-medium text-gray-900 focus:border-[#ff5f35] focus:outline-none focus:ring-2 focus:ring-[#ff5f35]/20" />
        </label>}
      </section>}

      <section className="space-y-5 rounded-3xl border border-gray-100 bg-white p-5 shadow-xs sm:p-6">
        <div><h3 className="text-lg font-black text-gray-950">디자인</h3><p className="mt-1 text-xs text-gray-400">{isCollection ? `내부 링크 ${link.links?.length || 0}개에 함께 적용됩니다.` : '이 항목에만 적용됩니다.'}</p></div>
          <section className="space-y-3"><h4 className="text-sm font-black text-gray-900">기본 색상</h4><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            {variant !== 'reservation' && variant !== 'map' && <details className="group sm:col-span-2 rounded-2xl border border-gray-200 bg-gray-50/70"><summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-xs font-black text-gray-700">아이콘 색상 설정<ChevronDown className="h-4 w-4 transition group-open:rotate-180" /></summary><div className="grid grid-cols-1 gap-3 border-t border-gray-200 p-4 sm:grid-cols-2"><div className="space-y-2 text-xs font-bold text-gray-600"><span>아이콘 색상</span><ColorPickerPopover label="아이콘 색상" value={effective.iconColor} opacity={effective.iconOpacity} onChange={(iconColor) => updateStyle({ iconColor })} onOpacityChange={(iconOpacity) => updateStyle({ iconColor: effective.iconColor, iconOpacity })} suggested={['#111827', '#FFFFFF', '#7C3AED', '#2563EB', '#DC2626', '#059669', '#F59E0B']} /></div><div className="space-y-2 text-xs font-bold text-gray-600"><span>아이콘 배경색</span><ColorPickerPopover label="아이콘 배경색" value={effective.iconBackgroundColor} opacity={effective.iconBackgroundOpacity} onChange={(iconBackgroundColor) => updateStyle({ iconBackgroundColor })} onOpacityChange={(iconBackgroundOpacity) => updateStyle({ iconBackgroundColor: effective.iconBackgroundColor, iconBackgroundOpacity })} suggested={['#000000', '#FFFFFF', '#7C3AED', '#2563EB', '#FDE68A', '#D1FAE5', '#FCE7F3']} /></div></div></details>}
          </div></section>

          {variant === 'reservation' && <section className="space-y-4 rounded-2xl border border-[#171714]/15 bg-[#f4f1e8] p-4"><div><h4 className="text-sm font-black text-gray-900">캘린더 버튼 스타일</h4><p className="mt-1 text-xs font-medium text-gray-500">일정 카드, 이전·다음 버튼, 일정이 있는 날짜와 표시 색상에 한꺼번에 적용됩니다.</p></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          {variant === 'form' && <section className="space-y-4 rounded-2xl border border-[#171714]/15 bg-[#f4f1e8] p-4"><div><h4 className="text-sm font-black text-gray-900">제출 버튼 스타일</h4><p className="mt-1 text-xs font-medium text-gray-500">방문자가 정보를 보낼 때 누르는 버튼에 적용됩니다.</p></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <section className="space-y-3"><h4 className="text-sm font-black text-gray-900">글자</h4><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          </div><label className="space-y-2 block text-xs font-bold text-gray-600">글자 크기 <span className="float-right text-gray-900">{effective.fontSize}px</span><input type="range" min="11" max="24" value={effective.fontSize} onChange={(e) => updateStyle({ fontSize: Number(e.target.value) })} className="w-full accent-[#ff5f35]" /></label></section>

          <details className="group rounded-2xl border border-gray-200 bg-gray-50/70"><summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-black text-gray-800">테두리·그림자<ChevronDown className="h-4 w-4 transition group-open:rotate-180" /></summary><section className="space-y-4 border-t border-gray-200 p-4"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          </div><label className="space-y-2 block text-xs font-bold text-gray-600">테두리 두께 <span className="float-right text-gray-900">{effective.borderWidth}px</span><input type="range" min="0" max="8" value={effective.borderWidth} onChange={(e) => updateStyle({ borderWidth: Number(e.target.value) })} className="w-full accent-[#ff5f35]" /></label><label className="space-y-2 block text-xs font-bold text-gray-600">모서리 둥글기 <span className="float-right text-gray-900">{effective.borderRadius >= 999 ? '완전 둥글게' : `${effective.borderRadius}px`}</span><input type="range" min="0" max="60" value={Math.min(effective.borderRadius, 60)} onChange={(e) => updateStyle({ borderRadius: Number(e.target.value) })} className="w-full accent-[#ff5f35]" /></label></section></details>
