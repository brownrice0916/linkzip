...
type EditorVariant = 'button' | 'group-list' | 'group-grid' | 'reservation' | 'social' | 'form';
...
const getDefaults = (variant: EditorVariant, theme: ThemeDefaults): EffectiveDefaults => {
  ...
  if (variant === 'group-list' || variant === 'group-grid') {
    return { backgroundColor: '#ffffff', textColor: '#111827', borderColor: '#e5e7eb', borderWidth: 1, borderRadius: variant === 'group-grid' ? 16 : 999, fontSize: variant === 'group-grid' ? 12 : 15, fontWeight: 700, shadow: 'soft' };
  }
  ...
  return (
    <div className="space-y-7 animate-fade-in pb-20 font-sans">
      <header className="flex items-center gap-4">
        ...
        <div className="min-w-0"><h2 className="text-2xl font-black text-gray-950">링크 상세</h2><p className="text-xs font-bold text-purple-600 mt-0.5">{variantLabel[variant]}</p></div>
      </header>
      ...
      <section className="bg-white rounded-3xl border border-gray-100 shadow-xs p-6 space-y-6">
        <div><h3 className="text-xl font-black text-gray-950">디자인</h3><p className="text-xs text-gray-400 mt-1">이 항목에만 적용되는 스타일입니다.</p></div>
        <div className="space-y-3"><h4 className="text-sm font-black text-gray-900">실제 형태 미리보기</h4><div className="p-6 rounded-3xl bg-gray-100">{renderPreview()}</div></div>
          ...
          <section className="space-y-4"><h4 className="text-sm font-black text-gray-900">색상</h4><div className="grid grid-cols-2 gap-3">
            <label className="space-y-2 text-xs font-bold text-gray-600">{variant === 'button' || variant.startsWith('group') ? '버튼 배경색' : '카드 배경색'}<div className="flex items-center gap-2 border border-gray-200 rounded-xl p-2"><input type="color" value={effective.backgroundColor} onChange={(e) => onUpdate({ buttonColor: e.target.value })} className="w-8 h-8 border-0 bg-transparent cursor-pointer" /><span className="font-mono text-[11px]">{link.buttonColor || effective.backgroundColor}</span></div></label>
            <label className="space-y-2 text-xs font-bold text-gray-600">글자색<div className="flex items-center gap-2 border border-gray-200 rounded-xl p-2"><input type="color" value={effective.textColor} onChange={(e) => onUpdate({ buttonTextColor: e.target.value })} className="w-8 h-8 border-0 bg-transparent cursor-pointer" /><span className="font-mono text-[11px]">{link.buttonTextColor || effective.textColor}</span></div></label>
          </div></section>
          ...
          <section className="space-y-4"><h4 className="text-sm font-black text-gray-900">{variant === 'button' || variant.startsWith('group') ? '버튼 박스' : '카드 박스'}</h4><div className="grid grid-cols-2 gap-3">
            <label className="space-y-2 text-xs font-bold text-gray-600">테두리 색<input type="color" value={effective.borderColor} onChange={(e) => updateStyle({ borderColor: e.target.value })} className="w-full h-11 rounded-xl border border-gray-200 p-1 bg-white cursor-pointer" /></label>
          ...
          </div><label className="space-y-2 block text-xs font-bold text-gray-600">테두리 두께 <span className="float-right text-gray-900">{effective.borderWidth}px</span><input type="range" min="0" max="8" value={effective.borderWidth} onChange={(e) => updateStyle({ borderWidth: Number(e.target.value) })} className="w-full accent-purple-600" /></label><label className="space-y-2 block text-xs font-bold text-gray-600">모서리 둥글기 <span className="float-right text-gray-900">{effective.borderRadius >= 999 ? '완전 둥글게' : `${effective.borderRadius}px`}</span><input type="range" min="0" max="60" value={Math.min(effective.borderRadius, 60)} onChange={(e) => updateStyle({ borderRadius: Number(e.target.value) })} className="w-full accent-purple-600" /></label><label className="space-y-2 block text-xs font-bold text-gray-600">투명도 <span className="float-right text-gray-900">{effective.opacity}%</span><input type="range" min="10" max="100" value={effective.opacity} onChange={(e) => updateStyle({ opacity: Number(e.target.value) })} className="w-full accent-purple-600" /></label></section>

        <button type="button" onClick={() => onUpdate({ buttonColor: undefined, buttonTextColor: undefined, customStyle: undefined })} className="w-full py-3 rounded-2xl border border-gray-300 text-xs font-black text-gray-700 hover:bg-gray-50 flex items-center justify-center gap-2 cursor-pointer"><RotateCcw className="w-4 h-4" /> {variantLabel[variant]} 기본값으로 초기화</button>
      </section>
    </div>
  );
};
