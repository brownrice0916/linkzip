...
const BusinessFooter: React.FC<BusinessFooterProps> = ({ compact = false, dark = false }) => {
  const linkClass = compact
    ? 'text-current opacity-75 transition hover:opacity-100 hover:underline'
    : dark
      ? 'text-gray-300 transition hover:text-white hover:underline'
      : 'text-gray-600 transition hover:text-gray-950 hover:underline';

  return (
    <footer className={compact ? 'w-full px-3 pb-5 pt-6 text-center' : 'relative z-10 border-t border-white/10 px-6 py-8 text-center'}>
